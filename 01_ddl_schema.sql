...
SELECT 
    ...
    o.payment_method,
    o.order_status,
    TIMESTAMPDIFF(MINUTE, o.order_timestamp, o.actual_ready_time) AS prep_time_minutes
FROM FACT_ORDER o
LEFT JOIN DIM_CUSTOMER c ON o.customer_id = c.customer_id
LEFT JOIN DIM_LOCATION l ON o.location_id = l.location_id;
...
SELECT 
    ...
    m.base_price,
    m.cost_to_make,
    m.base_price - m.cost_to_make AS profit_margin,
    COUNT(DISTINCT oi.order_id) AS times_ordered,
    SUM(oi.quantity) AS total_quantity_sold,
    SUM(oi.line_total) AS total_revenue,
    AVG(oi.line_total) AS avg_line_total
FROM DIM_MENU_ITEM m
JOIN DIM_CATEGORY cat ON m.category_id = cat.category_id
LEFT JOIN FACT_ORDER_ITEM oi ON m.item_id = oi.item_id
GROUP BY m.item_id, m.item_name, cat.category_name, m.base_price, m.cost_to_make;
